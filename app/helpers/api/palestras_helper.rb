module Api::PalestrasHelper
  ...
  def transform (list)
    ...
    palestras = shifts_180(palestras)
    
    #Retorna uma array de tracks. Cada track tem uma array de palestras
    distribuir_excedente(palestras)
    
    palestras = tracks_time(palestras)
  ...
  def tracks_time(tracks) # adiciona almoço e evento e início de cada evento
    ...
    tracks.each_cons(2) do |track|  # percorre tracks (DIA)
      ...
      index += 1
      retorno[turno.to_sym] = track.flatten
      turno.succ!
    end
    retorno
  end
